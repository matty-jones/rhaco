...
defaults_dict = {
    "stoichiometry": {"Mo": 1, "V": 0.15, "Nb": 0.13, "Te": 0.12},
    "dimensions": [1, 1, 1],
        ...
        for z_repeat in range(surface_dimensions[2]):
            ...
            for y_repeat in range(surface_dimensions[1]):
                ...
                for x_repeat in range(surface_dimensions[0]):
                    print(
                        "\rAdding "
                        + repr([x_repeat, y_repeat, z_repeat])
                        + " to system...",
                        end=" ",
                    )
                    current_cell = crystal_unit_cell(template, stoichiometry_dict)
                    current_row.append(current_cell)
                    current_cell.translate(
                        [x_repeat * x_extent, y_repeat * y_extent, z_repeat * z_extent]
                    )
            ...
            for y_coord in range(surface_dimensions[1]):
                ...
                for x_coord in range(surface_dimensions[0]):
                    ...
                    if (
                        crystal_bonds
                        and (x_coord + 1 < surface_dimensions[0])
...
class mbuild_template(mb.Compound):
    ...
    def __init__(self, template, rigid, rolling_rigid_body_index):
        ...
        atom_types = [
            atom_type.split("[")[0]
            for atom_type in molecule.labels.keys()
            if "[" in atom_type
        ]
...
def parse_forcefields(forcefield_string):
    ...
    for forcefield in forcefield_string:
        ...
        if ((forcefield[0] == "'") and (forcefield[-1] == "'")) or (
            (forcefield[0] == '"') and (forcefield[-1] == '"')
        ):
            # Cut out the ' or "
        ...
        if not forcefield_exists:
            ...
            print(
                "Forcefield",
                forcefield,
                "not found with any compatible extension:",
                repr(PERMITTED_FF_FORMATS),
                "in either the FF_LIBRARY dir",
                FF_LIBRARY,
                "or cwd.",
            )
    ...
    if len(foyer_forcefield_list) > 1:
        print(
            "The following forcefields will be implemented with Foyer:",
            repr(foyer_forcefield_list),
        )
    elif len(foyer_forcefield_list) == 1:
        print(
            "The following forcefield will be implemented with Foyer:",
            foyer_forcefield_list[0],
        )
    if len(external_forcefield_list) > 1:
        print(
            "The following files will be used as forcefields:",
            repr(external_forcefield_list),
        )
    elif len(external_forcefield_list) == 1:
        print(
            "The following file will be used as a forcefield:",
            external_forcefield_list[0],
        )
...
def create_morphology(args):
    ...
    if (args.forcefield is None) or (
        (len(args.forcefield[0]) == 0) and (len(args.forcefield[1]) == 0)
    ):
...
def check_bonds(morphology, bond_dict, box_dims):
    for bond in morphology["bond_text"]:
        ...
        +(np.array(list(map(float, morphology["image_text"][int(bond[1])]))) * box_dims)
        posn2 = np.array(list(map(float, morphology["position_text"][int(bond[2])])))
        +(np.array(list(map(float, morphology["image_text"][int(bond[2])]))) * box_dims)
        delta_position = posn1 - posn2
        out_of_range = [
            np.abs(delta_position[axis]) > box_dims[axis] / 2.0 for axis in range(3)
        ]
        if any(out_of_range):
            print(
                "Periodic bond found:",
                bond,
                "because delta_position =",
                delta_position,
                ">=",
                box_dims,
                "/ 2.0",
...
def get_bond_dict(morphology):
    bond_dict = {
        atom_id: [] for atom_id, atom_type in enumerate(morphology["type_text"])
    }
    for bond in morphology["bond_text"]:
...
def check_wrapped_positions(input_dictionary):
    box_dims = [
        float(input_dictionary["box_attrib"][axis]) for axis in ["lx", "ly", "lz"]
    ]
    atom_positions = np.array(
        [np.array(list(map(float, _))) for _ in input_dictionary["position_text"]]
    )
    atom_images = np.array(
        [np.array(list(map(int, _))) for _ in input_dictionary["image_text"]]
    )
    xhi = box_dims[0] / 2.0
    xlo = -box_dims[0] / 2.0
    yhi = box_dims[1] / 2.0
    ...
    for atom_ID in range(len(atom_positions)):
        ...
        while atom_positions[atom_ID][2] < zlo:
            atom_positions[atom_ID][2] += box_dims[2]
            atom_images[atom_ID][2] -= 1
    input_dictionary["position_text"] = list(
        [list(map(str, _)) for _ in atom_positions]
    )
    input_dictionary["image_text"] = list([list(map(str, _)) for _ in atom_images])
...
def write_morphology_xml(morphology_dictionary, output_file_name):
    ...
    # Build the xml tree.
    root = ET.Element(
        morphology_dictionary["root_tag"], **morphology_dictionary["root_attrib"]
    )
    root.text = morphology_dictionary["root_text"]
    config = ET.Element(
        morphology_dictionary["config_tag"], **morphology_dictionary["config_attrib"]
    )
    ...
    all_child_tags = [
        ...
        for key in morphology_dictionary.keys()
        if "_".join(key.split("_")[:-1]) not in ["root", "config"]
    ]
    child_tags = []
    ...
    for child_tag in child_tags:
        ...
        if child_tag != "external_forcefields":
            ...
            data_to_write = "\n".join(
                [el for el in morphology_dictionary[child_tag + "_text"]]
            )
...
def fix_images(file_name):
    print("Fixing the images to ensure everything is wrapped within" " the box...")
...
def main():
    ...
    parser.add_argument(
        "-d",
        "--dimensions",
        type=lambda d: [int(dim) for dim in d.split("x") if len(dim) > 0],
        default=[1, 1, 1],
        required=False,
